...
function _handleOptions() {
  _handleOptions = _asyncToGenerator(function* (options) {
    Object.keys(CONFIG).forEach(k => {
      options[k] = options[k] || typeof options[k] === 'boolean' ? options[k] : CONFIG[k];
    });
    options.container = options.container() || CONFIG.container;
    var fixedType = CONFIG.fixedOptions.find(item => item === options.fixed.type);
    if (!fixedType) {
    ...
    options.speedOptions = options.speedOptions.map(sp => parseFloat(sp)).filter(sp => !isNaN(sp));
    if (options.speedOptions.length > 1) {
      options.speedOptions.sort((a, b) => a - b);
    }
    if (options.audio && options.audio.src) {
      options.audio = yield handleAudio(options.audio, options.parser);
    } else {
...
function _handleAudio() {
  _handleAudio = _asyncToGenerator(function* () {
    var audio = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
    var parser = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;
    var audioInfo = {};
    if (parser && (!audio.title || !audio.artist || !audio.cover)) {
      try {
        var {
          tags
        } = yield parseAudio(audio.src, parser);
...
function parseAudio(src, parser) {
...
class UI {
  ...
  initOptions(options) {
    this.el.style = "--color-primary: ".concat(options.themeColor, "; --color-handle-shadow: ").concat(options.themeColor, "cc");
...
class Player {
  ...
  created(options) {
    var _this = this;
    return _asyncToGenerator(function* () {
      _this.options = yield handleOptions(options);
      _this._renderComponents();
      _this.initUI(options);
      _this.initAudio();
      _this.events.trigger('inited');
      _this.ui.mount(_this.options.container);
    })();
  }
  get duration() {
  ...
  update(audio) {
    var _this2 = this;
    return _asyncToGenerator(function* () {
      _this2._audio = _this2._inited ? yield handleAudio(audio, _this2.options.parser) : audio;
      _this2._canplay = false;
      _this2.audio.src = _this2._audio.src;
      _this2.audio.title = _this2._audio.title;
      _this2.events.trigger('audioupdate', _this2._audio);
      _this2.ui.setAudioInfo(_this2._audio);
      if (_this2._hasMediaSession) {
        _this2.setMediaMetadata(_this2._audio);
      }
    })();
  }
  destroyAudio() {
